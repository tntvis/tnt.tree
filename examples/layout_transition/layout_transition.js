var tnt_theme_tree_layout_transition = function() {
	...
	var sel = menu_pane
		...
		var layout = tnt_tree.layout[this.value]().width(width).scale(scale)
	...
	tree_vis
	    .node_display(tnt_tree.node_display.triangle()
			  .size(5)
			  .stroke('black')
			  .fill('grey')
			 )
	    ...
	    .layout(tnt_tree.layout.radial().width(900).scale(scale))
	    .duration(2000);
